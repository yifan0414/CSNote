...
> [!abstract] 笔记定位
> 本节承接 [[02-RNN|02-RNN]] 与 [[03-LSTM与GRU|03-LSTM与GRU]]，用单层经典 RNN 推导最早期的 fixed-vector Seq2Seq。全文围绕同一个翻译样本展开：先定义 $P(y\mid x)$，再走完 Encoder → source summary → Decoder 的前向过程，随后分别完成训练和生成，最后从固定向量瓶颈自然引出 Attention。Padding、mask 与 shape 放在核心主线之后，作为工程实现层单独说明。
...
> | 类型 | 资料与本地文件 | 说明 |
> | --- | --- | --- |
> | 前置知识 | [[02-RNN\|02-RNN]] | RNN-LM、Teacher Forcing、BPTT 与生成 |
> | 前置知识 | [[03-LSTM与GRU\|03-LSTM与GRU]] | LSTM/GRU 及其状态接口 |
> | 课程课件 | [[cs224n-2026-lecture04-rnnlm.pdf\|CS224N Lecture 4: RNNLM]] | 经典 RNN Encoder-Decoder、训练与 bottleneck |
> | 课程讲义 | [[cs224n-2019-notes05-LM_RNN.pdf\|CS224N: Language Models, RNN, GRU and LSTM]] | RNN 与机器翻译背景 |
> | 下一阶段 | [[05-Attention\|05-Attention]] | 从固定摘要到动态读取，推导 Q/K/V 与 Self-Attention |
> | 延伸笔记 | [[从 Seq2Seq 到 Attention 再到 Transformer\|从 Seq2Seq 到 Attention 再到 Transformer]] | 从 fixed-vector 模型过渡到 Attention |
...
*图 1：Encoder 读完整个源句后，用最终状态条件化 Decoder；Decoder 从 `<START>`（即本文的 `<BOS>`）开始逐步生成。粉色反馈连线表示推理时把模型预测作为下一步输入。来源：[[cs224n-2026-lecture04-rnnlm.pdf#page=53|CS224N Lecture 4，PDF 第 53 页]]。*
...
*图 2：每个 Decoder 时间步产生一项 loss，梯度通过 Decoder、初始状态接口与 Encoder 端到端传播。图中下方目标 token 是 Teacher-Forced 输入。来源：[[cs224n-2026-lecture04-rnnlm.pdf#page=56|CS224N Lecture 4，PDF 第 56 页]]。*
...
*图 3：无论源句多长，全部源信息都必须先通过固定大小的最终 Encoder state，再进入 Decoder。来源：[[cs224n-2026-lecture04-rnnlm.pdf#page=58|CS224N Lecture 4，PDF 第 58 页]]。*
...
- [ ] 已回答自测问题
- 我仍不清楚的点：
- 与前置章节 [[02-RNN|02-RNN]]、[[03-LSTM与GRU|03-LSTM与GRU]] 的联系：
- 与下一阶段 [[05-Attention|05-Attention]] 的联系：
