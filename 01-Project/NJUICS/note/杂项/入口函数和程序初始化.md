...
![[attachments/e5611b2033154837ec7656db864a788b_MD5.png]]
...
- `pop %esi` 和 `mov %esp, %ecx`：在调用 `_start` 之前，**loader 会把用户参数和环境变量压入栈中** #pa/insight ，按照其压栈的方法，实际上栈顶的元素是 argc，而接下其下就是 argv 和环境变量的数组。下面是此时的栈布局，其中虚线箭头是执行 pop %esi 之前的栈顶（%esp），而实现箭头是执行之后的栈顶（%esp）。![[attachments/06c31360143fece8b8d3e3e0898ecc1f_MD5.png]]
