...
> [!tip] 阅读路线
> 本篇承接 [[Paper/架构学习/基础/04-Seq2Seq|Seq2Seq]]，重点回答三个问题：**为什么要按需读取、一次读取怎样计算、读取规则怎样学出来。**
>
...
若用 sampling，可以按 $p_t$ 采样下一个词；若用 beam search，需要为每条候选前缀维护各自的 Decoder 状态、累计分数和 query。不同候选可以共享源 K/V，但会形成各自的 Attention 权重。选择策略沿用 [[Paper/架构学习/基础/04-Seq2Seq|Seq2Seq]] 中的解码方法。
...
下一篇 [[Paper/架构学习/基础/06-transformer|Transformer]] 将沿着这些问题展开：用 Self-Attention 建立位置之间的联系，用多头提供多种读取方式，用位置信息表达顺序，再把这些操作组织成可训练的完整网络。

## 附录 A：复制、平均与混合稳定性（选读）

这组题围绕一个逐步加深的问题展开：**一次 Attention 能取回一个 value；如果希望同时取回两个，并且每次都按一半一半的比例混合，需要满足什么条件？**

下面参考 [[Paper/架构学习/基础/dive-to-attention/dive-to-attention|dive-to-attention]] 的讲解顺序，沿着“希望得到的输出 → 所需权重 → 怎样产生这些分数 → key 变化后会发生什么”展开。A.1、A.2 先分析固定 keys，A.3 引入混合比例问题，A.4 补充概率知识，A.5、A.6 比较两种噪声，最后在 A.7 连接到多头注意力。
...
**原题 1(a)：Copying in attention** · [[Paper/架构学习/CS224n/resources/assignments/a3.pdf#page=2|PDF 第 2 页]]
...
**原题 1(b)：An average of two** · [[Paper/架构学习/CS224n/resources/assignments/a3.pdf#page=2|PDF 第 2 页]]
...
**原题 1(c)(i)：各向同性小扰动** · [[Paper/架构学习/CS224n/resources/assignments/a3.pdf#page=2|PDF 第 2 页]]
...
**原题 1(c)(ii)：沿 Key 方向的扰动** · [[Paper/架构学习/CS224n/resources/assignments/a3.pdf#page=2|PDF 第 2 页题干]]

![[_assets/images/a3-1c-ii-axis-noise.png|900]]

**Figure 1 与续题** · [[Paper/架构学习/CS224n/resources/assignments/a3.pdf#page=3|PDF 第 3 页]]
...
**原题 2(a)：Permuting the input** · [[Paper/架构学习/CS224n/resources/assignments/a3.pdf#page=4|PDF 第 4 页]]
...
- [[Paper/架构学习/基础/dive-to-attention/dive-to-attention|dive-to-attention]]：附录讲解思路的参考，按输出目标、分数构造与扰动影响逐步展开。
- [[Paper/架构学习/CS224n/resources/assignments/a3.pdf|CS224N Winter 2026 Assignment 3]]：第 1(a)、1(b)、1(c) 题及第 2(a) 题的原始题干与截图。
