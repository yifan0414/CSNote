...
> [!abstract] 笔记定位
> 本节从**语言模型**出发，说明 n-gram 与固定窗口神经语言模型为什么需要截断上下文，再推导 vanilla RNN 如何用共享参数和递归 hidden state 处理任意长度序列。重点覆盖 RNN-LM 的前向计算、Teacher Forcing、BPTT、梯度消失/爆炸与困惑度。LSTM/GRU 和 Seq2Seq 只负责建立下一阶段接口，具体结构分别放在 [[Paper/架构学习/基础/03-LSTM与GRU.md|03-LSTM与GRU]] 与 [[Paper/架构学习/基础/04-Seq2Seq.md|04-Seq2Seq]]。
...
> | 类型 | 资料与本地文件 | 说明 |
> | --- | --- | --- |
> | 课程课件 | [[Paper/架构学习/CS224n/resources/slides/cs224n-2026-lecture04-rnnlm.pdf\|CS224N Lecture 4: Language Models and Recurrent Neural Networks]] | 本笔记的主要参考资料 |
> | 前置知识 | [[Paper/架构学习/基础/00-分词.md\|00-分词]] | 文本到 token IDs |
> | 前置知识 | [[Paper/架构学习/基础/01-词向量.md\|01-词向量]] | token IDs 到 embedding |
> | 下一阶段 | [[Paper/架构学习/基础/03-LSTM与GRU.md\|03-LSTM与GRU]] | 用门控改善递归记忆 |
> | 下一阶段 | [[Paper/架构学习/基础/04-Seq2Seq.md\|04-Seq2Seq]] | Encoder-Decoder 条件序列生成 |
...
*图 1：语言模型通过链式分解为整段文本分配概率；真正需要建模的是每一步的 next-token distribution。来源：[[Paper/架构学习/CS224n/resources/slides/cs224n-2026-lecture04-rnnlm.pdf#page=4|CS224N Lecture 4，PDF 第 4 页]]。*
...
*图 2：固定窗口神经语言模型把若干位置的 embedding 拼接后预测下一个词。来源：[[Paper/架构学习/CS224n/resources/slides/cs224n-2026-lecture04-rnnlm.pdf#page=21|CS224N Lecture 4，PDF 第 21 页]]。*
...
*图 3：RNN-LM 在时间上重复使用同一套 embedding、状态转移和输出参数。来源：[[Paper/架构学习/CS224n/resources/slides/cs224n-2026-lecture04-rnnlm.pdf#page=24|CS224N Lecture 4，PDF 第 24 页]]。*
...
*图 4：Teacher Forcing 用真实语料 token 驱动每个时间步，并把所有 next-token loss 汇总。来源：[[Paper/架构学习/CS224n/resources/slides/cs224n-2026-lecture04-rnnlm.pdf#page=31|CS224N Lecture 4，PDF 第 31 页]]。*
...
*图 5：Backpropagation Through Time 沿时间展开 RNN，并把共享权重在各时间步的梯度贡献累加。来源：[[Paper/架构学习/CS224n/resources/slides/cs224n-2026-lecture04-rnnlm.pdf#page=35|CS224N Lecture 4，PDF 第 35 页]]。*
...
*图 6：远处训练信号必须穿过一连串 recurrent Jacobian；当每一步都缩小时，信号越传越弱。来源：[[Paper/架构学习/CS224n/resources/slides/cs224n-2026-lecture04-rnnlm.pdf#page=45|CS224N Lecture 4，PDF 第 45 页]]。*
...
*图 7：梯度范数超过阈值时，按比例缩放后再执行参数更新。来源：[[Paper/架构学习/CS224n/resources/slides/cs224n-2026-lecture04-rnnlm.pdf#page=49|CS224N Lecture 4，PDF 第 49 页]]。*
...
> [!note] 本节只负责建立接口
> Seq2Seq 的状态连接、`<BOS>/<EOS>`、target shifting、Teacher Forcing、端到端训练、padding/masking、greedy/beam search 与固定向量瓶颈，统一放在 [[Paper/架构学习/基础/04-Seq2Seq.md|04-Seq2Seq]] 中展开。本节只需知道：RNN 可以成为 Encoder，也可以成为 Decoder 的 recurrent backbone。
...
| Seq2Seq 衔接 | RNN 可作为 Encoder 或条件自回归 Decoder | 详细机制见 [[Paper/架构学习/基础/04-Seq2Seq.md\|04-Seq2Seq]] |
...
> }
> $$
> RNN 用固定参数处理可变长度序列，但把信息和梯度都限制在逐步传递的链上。BPTT 让这条链可训练，梯度消失/爆炸揭示其优化困难；下一步先在 [[Paper/架构学习/基础/03-LSTM与GRU.md|03-LSTM与GRU]] 中学习可控记忆，再在 [[Paper/架构学习/基础/04-Seq2Seq.md|04-Seq2Seq]] 中学习如何把 Encoder 与条件自回归 Decoder 连接起来。
...
8. 最后只需理解 RNN 可以成为 Encoder 或条件自回归 Decoder，完整 Seq2Seq 留到 [[Paper/架构学习/基础/04-Seq2Seq.md|04-Seq2Seq]]。
...
- [ ] 已回答自测问题
- 我仍不清楚的点：
- 与前置章节 [[Paper/架构学习/基础/01-词向量.md|01-词向量]] 的联系：
- 与下一阶段 [[Paper/架构学习/基础/03-LSTM与GRU.md|03-LSTM与GRU]]、[[Paper/架构学习/基础/04-Seq2Seq.md|04-Seq2Seq]] 的联系：
