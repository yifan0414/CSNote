...
3. 沿 shape 完整推导一次 single-head 和 multi-head attention。
...
>
> $$
> q_i=[q_{i1},q_{i2},\ldots,q_{i128}],\qquad
> k_j=[k_{j1},k_{j2},\ldots,k_{j128}],
> $$
>
> 因此：
>
...
> 这个标量表示第 $i$ 个 query token 与第 $j$ 个 key token 的相关性。
>
...
在单头 attention 中，若投影维度为 $d_h$：

$$
Q,K,V\in\mathbb{R}^{B\times N\times d_h}.
$$

在多头 attention 中，通常先投影到 $D$，再 reshape 和 transpose：
...
其中 $M$ 是 additive mask：允许的位置加 0，禁止的位置加 $-\infty$。

### 例题 4：单头 Attention 手算
...
## 5.1 为什么 mask 必须在 softmax 前应用

attention mask 的目标是让非法位置获得严格为 0 的概率。常见做法是先把非法 score 加上 $-\infty$：
...
M_{ij}=
\begin{cases}
0,&\text{位置 }j\text{ 对 query }i\text{ 可见},\\
-\infty,&\text{位置 }j\text{ 不可见}.
\end{cases}
$$
...
非法位置不会参与 softmax 的分子和归一化分母。

如果先 softmax 再把某些权重设为 0，那么非法位置已经占用了部分概率质量，剩余合法位置的权重和可能小于 1。因此 mask 应在 softmax 前应用。

## 5.2 Causal mask

decoder-only 模型中，位置 $i$ 只能看到位置 $j\le i$。Additive causal mask 可写为：
...
行表示 query，列表示 key。第 1 个 token 只能看自己，第 4 个 token 可以看前 4 个位置。

## 5.3 Padding mask

batch 内不同序列通常会 pad 到相同长度。假设两条序列的有效长度分别为 4 和 2，padding 到 $N=4$，key padding 可见性为：
...
其常见 shape 为 $[B,1,1,N]$，会在 head 和 query 维广播。它主要防止有效 query 读取 padding key。

## 5.4 合并 causal mask 与 padding mask
...
若使用布尔可见性，可做逻辑与；若使用 additive mask，可将两个 mask 相加：

$$
M=M^{\text{causal}}+M^{\text{padding}}.
$$

对上例中第二条有效长度为 2 的序列，合并后的可见性矩阵为：
...
第 3、4 行对应 padding query。工程实现中通常还需要确保这些 query 的输出不影响训练，例如：

- 计算 loss 时忽略 padding target；
- 或额外屏蔽 padding query；
- 或将 padding query 的输出清零。

> [!warning] 全部被屏蔽的行
> 如果某一 query 行的所有 score 都是 $-\infty$，softmax 可能出现 NaN。实现时应保证有效 query 至少有一个可见 key，并单独处理 padding query。
...
> [!success]- 解答
> 1. 可以。它是典型 key padding mask，在 head 和 query 维广播。
> 2. 可以。它可表示所有样本、所有 head 共用的 causal mask。
> 3. 不能直接按预期与四维 score 相加。广播会从右向左对齐，语义不清晰；应显式扩成 $[8,1,1,128]$。
...
$$
A=\operatorname{softmax}(S,\text{key dimension})
\in\mathbb{R}^{B\times H\times N\times N}.
$$

### 第四步：每个 head 的 context
...
$$
[B,H,N,d_h]\rightarrow[B,N,H,d_h]\rightarrow[B,N,D].
$$

### 第六步：Output projection
