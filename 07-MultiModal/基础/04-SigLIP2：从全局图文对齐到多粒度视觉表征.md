...
![[_assets/images/siglip2-04-multigranular-framework.drawio.svg|900]]

> **SigLIP 2 让同一个视觉编码器做几类互补的题：判断整图与文本是否匹配，支持描述与区域位置预测，再用图像自身的教师目标学习局部语义。训练时增加辅助分支，使用时按任务取出全局向量或空间 tokens；数据筛选与输入适配则是另外两层改进。**

理解 SigLIP 2，可以像 BLIP 一样沿着四个问题展开：

| 核心问题 | 简要答案 |
| --- | --- |
| 监督信号从哪里来？ | 图文配对、描述及自动标注的区域—描述关系，以及同图不同视图上的 EMA teacher 目标 |
| 网络如何表示图文？ | 保留 ViT 与文本塔；未池化视觉 tokens 同时服务 LocCa decoder 和自监督辅助分支 |
| Loss 如何构造？ | Sigmoid 做逐对匹配，LocCa 做描述与坐标预测，自蒸馏和掩码预测做教师表示匹配 |
| 哪些参数与数据会更新？ | 各目标按阶段更新共享视觉塔，teacher 通过 EMA 更新；多语言混合与 ACID 改变训练数据，NaFlex 改变输入适配方式 |

下文依次解释：**配对与局部监督 → 双塔与辅助分支 → Sigmoid 对齐 → LocCa 描述与定位 → 视觉自监督 → 梯度与参数更新 → 数据选择 → 分辨率与 NaFlex → 下游使用与实验 → 完整心智模型**。

前置阅读：[[03-SigLIP：从逐对二分类到共享语义空间]]；组织方式参照 [[05-BLIP：从图文对齐到理解、生成与数据自举]]。

本文讨论原论文正式命名的 **SigLIP 2**，标题写作 SigLIP2 方便检索。示例与抽象公式用于解释机制；固定分辨率、NaFlex 与小模型微调是不同配方，不能把所有模块视为每个 checkpoint 的共同配置。
...
### 1.3 配对、伪标注与教师目标，都不是无误的真值

同一张狗图片可以提供不同层次的答案：网页描述提供整图语义，自动检测给出区域—描述关系，EMA teacher 提供表示目标。它们的来源不同，误差也不同：网页文本可能不准确，区域伪标注可能错位，教师表示也不是人工标注的分割图。

因此，新增目标的意义是**增加互补约束**，而不是把所有监督都变成可靠真值。接下来先看这些答案分别交给哪些网络模块处理，再逐项看 loss。

## 2. 网络架构：区分发布时的双塔与训练时的辅助分支

![[_assets/images/siglip2-01-2502.14786-training-overview.png|600]]

> [!info] 读图提示：百分比表示训练进程覆盖范围
...
> [!tip] 读架构时，先追踪取的是哪一种表示
> 把 MAP 想成“整图摘要”，把未池化 tokens 想成“仍保留位置索引的视觉特征集合”。检索通常比较摘要；定位或稠密任务则需要访问空间特征。这里的类比只帮助区分接口，不意味着每个 token 已经对应一个明确对象。
...
> [!warning] Tokenizer 不是语言模型
> **使用 Gemma tokenizer，不等于使用完整 Gemma 语言模型作为文本编码器。**

SigLIP 2 的文本塔仍是自己的 Transformer 编码分支；它不是负责对话的自回归大语言模型，也不是第 4 节的 LocCa decoder。
...
### 2.5 先把模块分清：共享的是视觉参数，不是所有文本模块

| 路径 | 读取什么？ | 输出交给谁？ |
| --- | --- | --- |
| 图像塔 + MAP | 原图的视觉 tokens | 全局图文匹配 |
| 独立文本塔 | 配对文本 | 全局图文匹配 |
| LocCa decoder | 视觉 tokens + 当前任务条件 | 描述或坐标预测目标 |
| Student 自监督辅助头 | 额外视图的整体或逐 patch 特征 | EMA teacher 提供的表示目标 |

**文本塔负责“把文本编码成向量”，LocCa decoder 负责“读图后预测序列”，EMA teacher 负责“提供视觉表示目标”。** 三者不要因为都参与训练，就混成同一个模块。与 BLIP 的 MED 不同，这里不能套用“文本侧切换运行模式并部分共享参数”的解释。

## 3. Sigmoid：先保留整图与文本的逐对匹配能力

### 3.1 先看题目：哪张图与哪段文字是原始配对？

狗图片与它的描述构成正配对，与 batch 中其他图片的描述交叉组合成负配对。这是沿用 SigLIP 的训练标签约定，不是说所有错配文本在语义上都必然错误。

网络仍先独立编码图像和文本，再比较全局向量；它不需要让文本逐 token 读取图片。**这条路径保住原来的全局检索能力，后面的辅助路径再补局部约束。**
...
> [!example] 用一个正配对检查梯度方向
> 假设某个正配对的 $y_{ij}=1$，当前预测为 $p_{ij}=0.2$，则 logit 梯度为 $-0.8/B$。若暂把 logit 当作可直接更新的量，梯度下降会把它推高；实际训练则通过链式法则更新编码器及打分参数。这是帮助理解符号方向的数值例子，不是论文实验结果。
...
> [!important] LocCa 不能完全套用 BLIP 的自回归 LM 解释
...
> [!warning] 训练过生成任务，不等于发布了生成模型
> 原论文明确说明，该 decoder 不包含在模型发布中。所以下载 SigLIP 2 双塔权重后，不能仅凭它曾经训练过 captioning，就认为它已经是可直接生成描述、坐标或对话的完整模型。

## 5. 自蒸馏与掩码预测：从图像自身构造局部学习目标

先把这部分与 LocCa 分开：LocCa 的答案是描述与坐标；这里的答案是 **teacher 生成的视觉表示**。下面按“看什么 → 答案从哪里来 → 如何比较”的顺序，分别理解视图级和位置级任务。

### 5.1 局部到全局：只看到一部分，预测整体语义目标
...
> [!note] Stop-gradient 与 EMA 说的是两件不同的事
> Stop-gradient 规定“这次反向传播不沿目标分支更新 teacher”；EMA 规定“teacher 的参数之后怎样跟随 student”。因此，停止梯度不等于 teacher 永远不变，也不等于 student 无法从 teacher 的目标中学习。
...
> [!warning] Masked prediction 不是像素重建，也不是 token 剪枝
> 这项任务需要区分三件事：
>
...
### 6.4 跟着一次定位错误走：为什么 decoder 的任务会改善 ViT？

假设 decoder 把“左边的狗”对应的框预测错了。LocCa loss 会调整 decoder 如何使用视觉特征，也会经 cross-attention 回传到被读取的视觉 tokens，再更新产生这些 tokens 的 ViT。共享视觉塔因此有机会学到更有利于区分对象与位置的表示。

同一阶段，全局图文目标仍要求整图向量与配对描述对齐；若已开启自监督，辅助头还会给视觉塔传回视图级与位置级梯度。**最终相加的是同一组参数收到的梯度，不是把几种任务的输出直接拼接。**

Teacher 在反向传播时提供停止梯度的目标，随后按 EMA 规则跟随 student；独立文本塔则不会因为 decoder 预测错了坐标而自动收到 LocCa 梯度。

## 7. 数据策略：先决定看什么，再为小模型选择练习题
...
### 7.5 为什么叫隐式蒸馏，而不是新增一个 logits loss？
...
### 7.6 与 BLIP 的 CapFilt 对照：改进数据，不等于同一种自举

| 问题 | BLIP CapFilt | SigLIP 2 ACID |
| --- | --- | --- |
| 主要做什么？ | 补写描述、过滤原始与合成文本 | 从候选图文中为当前 learner 选择训练样本 |
| 知识如何传递？ | 生成与匹配能力改变下一轮的数据集 | Reference teacher 通过选样影响 learner 看到的数据 |
| 后续如何训练？ | 用改进后的数据预训练新 BLIP | 部分固定分辨率小模型继续用 sigmoid loss 微调 |

> [!summary] 数据改进的共同点，不应掩盖具体机制的差别
> 共同点是“数据也属于训练方法”；区别是 **ACID 不是给每张图片补写 caption 的 CapFilt，也不是前文产生自监督目标的 EMA 分支**。

## 8. 分辨率与 NaFlex：把视觉 token 预算和图片长宽比拆开考虑

### 8.1 标准版本：每个固定分辨率对应自己的 checkpoint
...
### 8.2 NaFlex：固定 patch 大小，灵活选择网格形状和长度
...
### 8.4 位置编码、padding 与 attention mask 如何配套？
...
> [!warning] NaFlex 不是标准版所有训练模块的简单叠加
...
## 9. 从预训练到使用：按表示接口理解下游任务与实验

与 BLIP 按任务启用不同运行模式类似，使用 SigLIP 2 也要先选对路径；但这里发布的是编码器，不是可直接生成描述的完整模型。先区分全局 embedding 与空间 tokens，再看对应实验能支持什么结论。

### 9.1 先问自己：要全局分数，还是空间 tokens？
...
> [!note] 整体比较回答“有没有提升”，消融才帮助判断“谁带来提升”
...
### 9.3 稠密预测：不是只看 pooled embedding
...
### 9.5 Dense features 更好，不代表任意 patch 都是已校准的文本 embedding
...
> [!question] 自测：维度相同，就可以直接做 patch–text 匹配吗？
> 不能仅凭维度判断。维度相同只说明点积在数学上可计算；还要检查两种表示是否在同一个空间中接受过相应的对齐监督，以及所得分数是否经过下游验证。

### 9.6 NaFlex 不等于免费减少计算
...
## 10. 最后串起来：SigLIP 2 的完整心智模型

### 10.1 跟着同一张狗图片走一遍

1. **配对出题**：图片与原描述提供全局正配对，其他文本提供交叉负配对。
2. **共享编码**：ViT 产生空间 tokens，MAP 汇聚整图表示；文本塔独立编码描述，完成 sigmoid 匹配。
3. **增加具体任务**：LocCa decoder 读取空间 tokens，练习整图描述、描述到框、框到描述。
4. **后期增加视觉目标**：标准路线的 student 看局部裁剪或遮挡视图，匹配 EMA teacher 的整体或对应位置表示。
5. **汇合更新**：不同 loss 的梯度更新同一个视觉塔；teacher 通过 EMA 跟随，而不是被目标匹配 loss 直接优化。
6. **按需取用**：检索取整图向量，MLLM 或稠密任务取空间 tokens，并连接各自的下游模块。是否做 ACID、是否采用 NaFlex，要另看变体。
...
- [[03-SigLIP：从逐对二分类到共享语义空间|前篇：SigLIP 的逐对 BCE、bias、梯度与 batch 机制]]。
- [[01-CLIP：从图文配对到共享语义空间|前篇：CLIP 的双塔与共享语义空间]]。

顶部 SVG 为依据本文机制整理的原创框架图，可在 draw.io 中继续编辑；正文保留的原论文训练概览与实验表图片均来自 Michael Tschannen 等作者的 SigLIP 2 论文，按原文的 CC BY 4.0 许可保留署名与来源；正文的示例、符号展开与 Video-MLLM 联系用于帮助理解，不作为论文额外报告的实验结论。
